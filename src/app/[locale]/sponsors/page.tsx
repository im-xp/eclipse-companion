...
function PartnerCard({ partner, dict }: { partner: Partner; dict: Dict }) {
  const inner = (
    <>
      ...
      <div className="mt-3">
        ...
        <span className="mt-0.5 block text-xs text-moon-white/55">
          {/* sponsors.json stays English (it will be regenerated by a future
              sync); the dict maps known labels, unknown ones fall back. */}
          {dict.sponsors.categoryLabels[partner.category] ?? partner.category}
        </span>
      </div>
    </>
  );
...
export default async function SponsorsPage({ params }: PageProps) {
  ...
  return (
    <div className="container-page pt-12 pb-16 sm:pt-16">
      <p className="eyebrow text-aurora-cyan">{dict.sponsors.eyebrow}</p>
      ...
      {GROUPS.map((group) => (
        <section key={group.title} className="mt-12">
          <h2 className="eyebrow text-solar-corona">
            {dict.sponsors.groupTitles[group.title] ?? group.title}
          </h2>
          <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
            {group.partners.map((p) => (
              <PartnerCard key={p.name} partner={p} dict={dict} />
            ))}
          </div>
        </section>
      ))}
