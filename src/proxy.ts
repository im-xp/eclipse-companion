...
export const config = {
  // Exclude _next internals and anything with a file extension (sw.js,
  // manifest.webmanifest, favicon.ico, images, fonts) — those are real files,
  // not pages, and must never be locale-rewritten or gate-redirected.
  // NOTE: /api is NOT excluded — the staging gate must also cover API routes:
  // this is the guard that makes AUTH_MODE=list safe (list mode signs any
  // listed email into their real profile with no possession proof, so
  // /api/auth/* and /api/profile/* must not be openly reachable). The proxy
  // body still skips the locale rewrite for /api.
